...
@WebServlet("/Inport_duizhang")
public class Inport_duizhang extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {  
  
    private static final long serialVersionUID = 1L;  
    File tmpDir = null;//初始化上传文件的临时存放目录  
    File saveDir = null;//初始化上传文件后的保存目录  
  public Inport_duizhang() {  
    super();  
  }       
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {  
    try{  
        if(ServletFileUpload.isMultipartContent(request)){  
          ...
          while(fii.hasNext()){  
            ...
            if(!fis.isFormField() && fis.getName().length()>0){//过滤掉表单中非文件域  
                ...
                //将上传文件写入数据库
                {
        			...
        			al=gbb.getduizhang_xls("up_"+fileName);
        		}
          ...
          request.setAttribute("upr_duizhang",al);
          request.getRequestDispatcher("upr_duizhang.jsp").forward(request, response);  
        }  
    }catch(Exception e){  
        e.printStackTrace();  
    }  
