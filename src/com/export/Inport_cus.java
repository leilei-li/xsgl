...
@WebServlet("/Inport_cus")
public class Inport_cus extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {  
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {  
    try{  
        if(ServletFileUpload.isMultipartContent(request)){  
          ...
          while(fii.hasNext()){  
            ...
            if(!fis.isFormField() && fis.getName().length()>0){//过滤掉表单中非文件域  
                ...
                //将上传文件写入数据库
                {
        			...
        			al=gbb.getcus_xls("up_"+fileName,fileName);
        		}
          ...
          request.setAttribute("upr_cus",al);
          request.getRequestDispatcher("upr_cus.jsp").forward(request, response);  
        }  
    }catch(Exception e){  
        e.printStackTrace();  
    }  
