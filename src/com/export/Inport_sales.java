...
@WebServlet("/Inport_sales")
public class Inport_sales extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {  
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {  
    try{  
        if(ServletFileUpload.isMultipartContent(request)){  
          ...
          while(fii.hasNext()){  
            ...
            if(!fis.isFormField() && fis.getName().length()>0){//过滤掉表单中非文件域  
                ...
                //将上传文件写入数据库
                {
        			...
        			al=gbb.getsales_xls("up_"+fileName);
        		}
          ...
          request.setAttribute("upr_sales",al);
          request.getRequestDispatcher("upr_sales.jsp").forward(request, response);  
        }  
    }catch(Exception e){  
        e.printStackTrace();  
    }  
